...
static void init(ComponentId componentsId) {
  u8g2_t u8g2;
  ...
  kea_sc_Display2x4Max7219C_Context* context = MemoryUtils.malloc(sizeof(kea_sc_Display2x4Max7219C_Context));
  context -> u8g2 = u8g2;
  context -> dateSemaphore = MemoryUtils.xSemaphoreCreateMutexWrap();
  context -> date = UNIX_EPOCH;

  ComponentService.putContext(componentsId, context);

  MemoryUtils.xTaskCreate(&redraw, "Display2x4Max7219C_redraw", KEA_SC_2X4MAX7219DISAPLYC_TASK_STACK_DEPTH, context, KEA_SC_2X4MAX7219DISAPLYC_TASK_PRIORITY, NULL);
}

static void printFirstFloor(u8g2_t* u8g2, Date date, bool showColon) {
...
static u8g2_uint_t printSecondFloor(u8g2_t* u8g2, Date date, uint8_t offset) {
  const uint8_t maxDateStrLength = 255;

  char* template = MessageService.getTranslatedMessage(MESSAGE_DATE_TEMPLATE);
  char* dayOfWeek = MessageService.getTranslatedCapitalDayOfWeek(date.dayOfWeek);
  char* month = MessageService.getTranslatedMonth(date.month);
...
void setDate(ComponentId componentsId, Date date) {
  kea_sc_Display2x4Max7219C_Context* context = ComponentService.getContext(componentsId);
...
static void redraw(void *pvParameter) {
  ...
  while (true)
  {
    ...
    if(!DateSerivce.isDateValid(date)) {
      ErrorService.put(MessageService.getTranslatedMessage(MESSAGE_WRONG_DATE_FORMAT));
      continue;
    }
...
kea_sc_Display_n const kea_sc_Disaplay2x4Max7219 = {
  .init = init,
  .displayDate = setDate
};